# app/parsers/ardupilot_parser.py
import pandas as pd
    ...
    def extract_raw_df(self, file_path: str) -> pd.DataFrame:
        ...
        while True:
            ...
            if msg_type == "ATT":
                ...
                current_state["timestamp_sec"] = getattr(msg, "TimeUS", 0) / 1e6
                current_state["lat"] = getattr(msg, "Lat", None)
                current_state["lon"] = getattr(msg, "Lng", None)
                current_state["alt"] = getattr(msg, "Alt", None)
                records.append(dict(current_state))
        ...
        if not records:
            raise ValueError(f"No valid ArduPilot telemetry extracted from {file_path}")
